...
/************************************************/
class course		// class for course record
	 {
	  public:
		 char CID[25];
		 char Name[30];
...
class iobuf
	 {
		 ...
		 virtual int Pack(course&,fstream&)=0;
		 virtual int Unpack(course&,fstream&)=0;
	 };
...
class delmfld : public varlen
	 {
		 public:
		 int Pack(course&,fstream&);
		 int Unpack(course&,fstream&);
		};
//////////////////////////////////////////////////////////////////////////
btree bt;
SSET s;
...
//search for given record with CID as key
...
////////////////////////////////////////////////////////////////////////////
//pack the course into Buffer with delimter and write into file
int delmfld::Pack(course &std,fstream  &file)
{
  Clear();
  strcpy(Buffer,std.CID);
  strcat(Buffer,"|");
  strcat(Buffer,std.Name);
  strcat(Buffer,"|");
  strcat(Buffer,std.tutor);
  strcat(Buffer,"|");
  strcat(Buffer,std.lectures);
  strcat(Buffer,"|");
  strcat(Buffer,std.duration);
  strcat(Buffer,"|");
  strcat(Buffer,std.year);
  strcat(Buffer,"|");
...
//read record from file into Buffer and unpack into course
int delmfld::Unpack(course &std,fstream &file)
{
  ...
  {
    int p=0,q;
    for(q=0;Buffer[q]!='|';q++) std.CID[p++]=Buffer[q];
    std.CID[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.Name[p++]=Buffer[q];
    std.Name[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.tutor[p++]=Buffer[q];
    std.tutor[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.lectures[p++]=Buffer[q];
    std.lectures[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.duration[p++]=Buffer[q];
    std.duration[p]='\0'; p=0; q++;
    for(;Buffer[q]!='|';q++) std.year[p++]=Buffer[q];
    std.year[p]='\0'; p=0; q++;
...
void course::Input(int flUpd)
{
   ...
   cout<<"\n\t lectures : "; cin>>lectures;
   
   
   cout<<"\n\t duration   : "; cin>>duration;
   cout<<"\n\t year   : ";cin>>year;
   cout<<"\n\t price    : ";cin>>price;
   }

//display 1 course record
void course::Display()
{
  cout << "\n\n\t course-Id     : " << CID
	   << "\n\n\t Name      : " << Name
...
//assign given course to this course
void course::Assign(course &std)
{
  strcpy(CID,std.CID);
  strcpy(Name,std.Name);
  strcpy(tutor,std.tutor);
  strcpy(lectures,std.lectures);
  strcpy(duration,std.duration);
  strcpy(year,std.year);
  strcpy(price,std.price);
}
//packs course into new buffer
int course::Pack(fstream &file)
{
  delmfld buffer;
  return (buffer.Pack(*this,file));
}
//unpacks buffer into course
int course::Unpack(fstream &file)
{
  delmfld buffer;
...
int main ()
{
   ...
   clrscr();
	...
	do
	{
	  ...
	  cout<<"******************************************";gotoxy(18,8);
	  cout<<"* 1: Display all records using B-Tree";gotoxy(59,8);cout<<"*";gotoxy(18,10);
	  cout<<"* 2: Add record into the file";gotoxy(59,10);cout<<"*";gotoxy(18,12);
	  cout<<"* 3: Search for record using B-Tree";gotoxy(59,12);cout<<"*";gotoxy(18,14);
	  cout<<"* 4: Delete record";gotoxy(59,14);cout<<"*";gotoxy(18,16);
	  cout<<"* 5: Update record";gotoxy(59,16);cout<<"*";gotoxy(18,18);
	  cout<<"* 6: Quit program";gotoxy(59,18);cout<<"*";gotoxy(18,20);
	  cout<<"******************************************"<<endl;
	  cout<<"\n\t\t Enter choice [1-6] : ";
	 cin>>ch;
	 switch(ch)
